...
public class ViewController {
    ...
    @GetMapping(value = "/todos/{todoId}/tightCouplingEvents", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public Flux<ServerSentEvent<TightCouplingEvent>> streamTightCouplingEvents(@PathVariable Long todoId,
                                                                               HttpServletRequest request) {
        // Stream the events from MongoDB
        Flux<TightCouplingEvent> events = eventRepository.findByTodoId(todoId);
